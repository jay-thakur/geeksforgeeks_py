...
Explanation:
Testcase 1: Each number can be expressed as sum of different numbers less than or equal to K as 5 (3 + 2),
8 (3 + 3 + 2), 10 (3 + 3 + 3 + 1), 13 (3 + 3 + 3 + 3 + 1). So, the sum of count of each element is 14.
"""


def total_count(arr, k):
    c = 0
    for i in arr:
        if i % k == 0:
            c += (i // k)
